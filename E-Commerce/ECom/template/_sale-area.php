<section id ="sale">
    <div class="container py-5">
        <h4 class="font-rubik font-size-20">ON SALE!!</h4>
        <hr>
        <div class="owl-carousel owl-theme">
            <div class="cars py-3">
                <div class="item font-rale">
                    <div class="d-flex flex-column">
<<<<<<< Updated upstream
                        <a><img src="../ECom/assets/cars/TOYOTA%20CAMRY.jpg" alt="cars1" class="img-fluid"</a>
                        <div class="text-center ">
                            <h6>TOYOTA CAMRY</h6>
                            <div class="stars text-warning font-size-12">
                                <span><i class="fas fa-star"></i></span>
                                <span><i class="fas fa-star"></i></span>
                                <span><i class="fas fa-star"></i></span>
                                <span><i class="fas fa-star"></i></span>
                                <span><i class="far fa-star"></i></span>
                            </div>
=======
                        <a><img src="./assets/cars/FJ%20Cruiser.jpg" alt="cars1" class="img-fluid"  </a>
                        <div class="text-center py-5">
                            <h6>TOYOTA Cruiser</h6>
>>>>>>> Stashed changes
                            <div class="price py-2">
                                <span>$3000</span>
                            </div>
                            <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                        </div>
                    </div>
                </div>
            </div>
<<<<<<< Updated upstream
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/SUBARU%20LEGACY.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>SUBARU LEGACY</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/NISSAN%20ALTIMA.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>NISSAN ALTIMA</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/MAZDA%206.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>MAZDA 6</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/KIA%20K5.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>KIA K5</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/HYUNDAI%20ACCENT.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>HYUNDAI ACCENT</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/HONDA%20ACCORD.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>HONDA ACCORD</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
            <div class="cars py-2">
                <div class="item font-rale">
                    <a><img src="../ECom/assets/cars/CHEVROLET%20MALIBU.jpg" alt="cars1" class="img-fluid"</a>
                    <div class="text-center ">
                        <h6>CHEVROLET MALIBU</h6>
                        <div class="stars text-warning font-size-12">
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="fas fa-star"></i></span>
                            <span><i class="far fa-star"></i></span>
                        </div>
                        <div class="price py-2">
                            <span>$3000</span>
                        </div>
                        <button type="submit" class="btn btn-warning font-size-12">Add to Cart</button>
                    </div>
                </div>
            </div>
=======









>>>>>>> Stashed changes
        </div>
    </div>
</section>