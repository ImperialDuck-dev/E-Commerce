<?php
require('./template/top.admin.php');

if(isset($_GET['type']) && $_GET['type']!=''){
    $type=get_safe_value($con,$_GET['type']);
    if($type=='status'){
        $operation=get_safe_value($con,$_GET['operation']);
        $id=get_safe_value($con,$_GET['id']);
        if($operation=='active'){
            $status='1';
        }else{
            $status='0';
        }
        $update_status_sql="update `montemayor car dealership`.cars_products set status='$status' where item_id='$id'";
        mysqli_query($con,$update_status_sql);
    }

    if($type=='delete'){
        $id=get_safe_value($con,$_GET['id']);
        $delete_sql="delete from `montemayor car dealership`.cars_products where item_id='$id'";
        mysqli_query($con,$delete_sql);
    }
}

$sql="select * from `montemayor car dealership`.cars_products";
$res=mysqli_query($con,$sql);
?>
    <div class="content pb-0">
        <div class="orders">
            <div class="row">
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="box-title">Products </h4>
                            <h4 class="box-link"><a href="manage-cars.php">Add Product</a> </h4>
                        </div>
                        <div class="card-body--">
                            <div class="table-stats order-table ov-h">
                                <table class="table ">
                                    <thead>
                                    <tr>
                                        <th class="serial">#</th>
                                        <th>ID</th>
                                        <th>Category</th>
                                        <th>Brand</th>
                                        <th>Name</th>
                                        <th>Image</th>
                                        <th>MRP</th>
                                        <th>Price</th>
                                        <th>Qty</th>
                                        <th>Register</th>
                                        <th>Short Description</th>
                                        <th>Description</th>
                                        <th></th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                    $i=1;
                                    while($row=mysqli_fetch_assoc($res)){?>
                                        <tr>
                                            <td class="serial"><?php echo $i?></td>
                                            <td><?php echo $row['item_id']?></td>
                                            <td><?php echo $row['category_id']?></td>
                                            <td><?php echo $row['item_brand']?></td>
                                            <td><?php echo $row['item_name']?></td>
                                            <td><img src="<?php echo $row['item_image']?>"/></td>
                                            <td><?php echo $row['mr_price']?></td>
                                            <td><?php echo $row['item_price']?></td>
                                            <td><?php echo $row['qty']?></td>
                                            <td><?php echo $row['item_register']?></td>
                                            <td><?php echo $row['starting_description']?></td>
                                            <td><?php echo $row['long_description']?></td>
                                            <td>
                                                <?php
                                                if($row['status']==1){
                                                    echo "<span class='badge badge-complete'><a href='?type=status&operation=deactive&id=".$row['item_id']."'>Active</a></span>&nbsp;";
                                                }else{
                                                    echo "<span class='badge badge-pending'><a href='?type=status&operation=active&id=".$row['item_id']."'>Deactive</a></span>&nbsp;";
                                                }
                                                echo "<span class='badge badge-edit'><a href='manage_product.php?id=".$row['item_id']."'>Edit</a></span>&nbsp;";

                                                echo "<span class='badge badge-delete'><a href='?type=delete&id=".$row['item_id']."'>Delete</a></span>";

                                                ?>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php
require('./template/footer.admin.php');
?>